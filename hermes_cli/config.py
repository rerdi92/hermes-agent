...
DEFAULT_CONFIG = {
    ...
    "eval_gate": {
        # Deterministic pre-dispatch policy/audit gate for Gateway and cron.
        # Default is fully inert: disabled and audit-only. To activate, set
        # enabled plus the specific surface(s); to block, also set enforce=true
        # and audit_only=false. ``approval_evidence`` can hold an exact approval
        # phrase such as "Approval required: connect eval gate pre-dispatch hook
        # to Gateway/cron" for controlled live enablement.
        "enabled": False,
