...
def _added_line_security_scan_command() -> str:
    return (
        ...
        "repo_env_names={'GIT_DIR','GIT_WORK_TREE','GIT_COMMON_DIR','GIT_OBJECT_DIRECTORY','GIT_ALTERNATE_OBJECT_DIRECTORIES','GIT_INDEX_FILE','GIT_NAMESPACE','GIT_PREFIX','GIT_CEILING_DIRECTORIES','GIT_DISCOVERY_ACROSS_FILESYSTEM','GIT_IMPLICIT_WORK_TREE'}\n"
        ...
        "            scan_body(line[1:])\n"
        "raw_top=git_output(['git','rev-parse','--show-toplevel'])\n"
        "repo_bound=False\n"
        ...
        "    raise SystemExit(1)\n"
        "inventory=git_output(['git','ls-files','--cached','--others','--exclude-standard','-z'])\n"
        "attributes=git_output(['git','check-attr','-z','--stdin','filter'], inventory) if inventory is not None else None\n"
        "active_filter=attributes is None\n"
        "if attributes is not None:\n"
        "    parts=attributes.split(b'\\0')\n"
        "    if parts and parts[-1] == b'':\n"
        "        parts.pop()\n"
        "    active_filter=len(parts) % 3 != 0 or any(value not in {b'unspecified', b'unset'} for value in parts[2::3])\n"
        "if active_filter:\n"
        "    hits.append('redacted-hit')\n"
        "    print('credential_like_added_assignments=', hits)\n"
        "    raise SystemExit(1)\n"
        "diff_options=['--no-ext-diff','--no-textconv','--no-color','--output-indicator-new=+','--output-indicator-old=-','--output-indicator-context= ']\n"
        "for args in (['git','diff',*diff_options,'--cached'], ['git','diff',*diff_options]):\n"
        ...
        "raw_paths=git_output(['git','ls-files','--others','--exclude-standard','-z']) or b''\n"
