...
class TestClarifyPrimitive:
    ...
    def test_resolve_text_response_rejects_custom_when_other_disallowed(self):
        ...
        cm.register("id-no-other", "session-no-other", "Pick", ["X", "Y"], allow_other=False)
        assert cm.resolve_text_response_for_session("session-no-other", "not offered") is False
        with cm._lock:
    ...
    def test_multi_select_text_response_enforces_min_max_and_allow_other(self):
        ...
        assert cm.resolve_text_response_for_session("session-ms-bounds", "1") is False
        assert cm.resolve_text_response_for_session("session-ms-bounds", "1,2,3") is False
        assert cm.resolve_text_response_for_session("session-ms-bounds", "custom") is False
        assert cm.resolve_text_response_for_session("session-ms-bounds", "1,3") is True
