...
import { type ComponentProps, type FormEvent, type KeyboardEvent, useCallback, useEffect, useMemo, useRef, useState } from 'react'
...
function ClarifyToolPending({ args }: ToolCallMessagePartProps) {
  ...
  useEffect(() => {
    if (!ready || submitting || expired || !hasChoices) {
      return
    }
    ...
    const onKeyDown = (event: globalThis.KeyboardEvent) => {
      ...
      if (event.key === 'Enter') {
        if (multiSelect && canSubmitSelected) {
          event.preventDefault()
          submitSelected()
        }
      ...
      } else if (allowOther && index === choices.length) {
        event.preventDefault()
