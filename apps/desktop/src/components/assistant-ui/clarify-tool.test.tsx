...
describe('ClarifyTool selection status UX', () => {
  ...
  it('submits a single-choice option from its letter shortcut', async () => {
    const request = renderClarifyTool()
  ...
  it('uses letter shortcuts to stage multi-select choices and Enter to submit them', async () => {
    const request = renderClarifyTool(vi.fn().mockResolvedValue({ ok: true }), { multiSelect: true })
  ...
  it('focuses the free-form Other field from the next letter shortcut', () => {
    renderClarifyTool()
